    ...
    internal class etc_1253
    {
        ...
        int ReadInt()
        {
            ...
            void GetRet()
            {
                ...
                while (q-- > 0)
                {
                    ...
                    else
                    {
                        ...
                        int f = ReadInt();
                        long ret = GetVal(S, E, chain[f].idx);
                        sw.Write($"{ret}\n");
                    }
                }
            }

            void Move(int _f, int _t)
            {
                ...
                while (chain[_f].dep < chain[_t].dep)
                {
                    ...
                    stk[len++] = (chain[chain[_t].head].idx, chain[_t].idx);
                    _t = chain[_t].parent;
                }
                ...
                while (chain[_f].dep > chain[_t].dep)
                {
                    ...
                    Update(S, E, chain[chain[_f].head].idx, chain[_f].idx, s, 1);

                    s += chain[_f].idx - chain[chain[_f].head].idx + 1;
                    _f = chain[_f].parent;
                }

                if (chain[_f].idx < chain[_t].idx)
                {

                    Update(S, E, chain[_f].idx, chain[_t].idx, s, 1);
                }
                else
                {

                    stk[len++] = (chain[_t].idx, chain[_f].idx);
                }
            ...
            void SetSeg()
            {
                ...
                seg = new (long val, long cnt)[1 << log];
                S = 1;
                E = n;
            }

            void Update(int _s, int _e, int _chkS, int _chkE, long _val, int _cnt, int _idx = 0)
            {
                ...
                Update(_s, mid, _chkS, _chkE, _val, _cnt, _idx * 2 + 1);

                if (_chkE <= mid || mid < _chkS)
                    Update(mid + 1, _e, _chkS, _chkE, _val, _cnt, _idx * 2 + 2);
                else
                {

                    long add = (mid + 1 - _chkS) * _cnt;
                    Update(mid + 1, _e, _chkS, _chkE, _val + add, _cnt, _idx * 2 + 2);
                }
            }
            ...
            long GetVal(int _s, int _e, int _chk, int _idx = 0)
            {
                ...
                long add = seg[_idx].val + (_chk - _s) * seg[_idx].cnt;
