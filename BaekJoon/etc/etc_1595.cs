...
내용 : Tricknology
    ...
    여기서 2k * x는 자명하게 짝수이다.
    k가 짝수라면 k가 짝수이므로 (2k - 1) * k 역시 짝수가 된다.
    k가 홀수라면 2k - 1은 짝수이고 (2k - 1) * k 역시 짝수가 된다.
    이는 2k * x + (2k - 1) * k이 2로 나눠떨어지므로 2이외에 소수가 될 수 없음을 뜻한다.
    그런데 x의 범위가 2 이상이므로 2k * x + (2k - 1) * k > 2가되어 소수가 될 수 없음을 뜻한다.
